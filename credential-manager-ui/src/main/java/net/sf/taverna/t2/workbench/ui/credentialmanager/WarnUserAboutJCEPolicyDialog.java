...
public class WarnUserAboutJCEPolicyDialog extends NonBlockedHelpEnabledDialog {
	...
	private void initComponents() {
		...
		messagePanel.setBorder(new CompoundBorder(new EmptyBorder(10, 10, 10,
				10), new EtchedBorder(EtchedBorder.LOWERED)));
	...
	public static File doNotWarnUserAboutJCEPolicyFile = new File(
	...
	public static void warnUserAboutJCEPolicy() {
		...
		try {
			final int maxAllowedKeyLength = Cipher
					.getMaxAllowedKeyLength("AES");
			alreadyInstalled = maxAllowedKeyLength > 128;
		} catch (NoSuchAlgorithmException e) {
			logger.error("Algorithm AES could not be found");
		}

		// Do not pop up a dialog if we are running headlessly.
		// If we have warned the user and they do not want us to remind them
		// again - exit.
		if (alreadyInstalled || warnedUser || GraphicsEnvironment.isHeadless()
				|| doNotWarnUserAboutJCEPolicyFile.exists()) {
			return;
		}
