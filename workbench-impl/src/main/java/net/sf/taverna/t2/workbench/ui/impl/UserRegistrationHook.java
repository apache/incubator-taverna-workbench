...
public class UserRegistrationHook implements StartupSPI {
	...
	public boolean startup() {
		...
		File registrationDataFile = new File(registrationDirectory, REGISTRATION_DATA_FILE_NAME);
		File doNotRegisterMeFile = new File(registrationDirectory, DO_NOT_REGISTER_ME_FILE_NAME);
		File remindMeLaterFile = new File(registrationDirectory, REMIND_ME_LATER_FILE_NAME);
		...
		// If user said to remind them - check if more than 2 weeks passed since we asked
		// previously.
		...
		FileFilter fileFilter = new FileFilter() {
			public boolean accept(File file) {
				return (!file.getName().equals(appHomeDirectory.getName()) // Exclude Taverna home
																			// directory for this
																			// app
						&& file.isDirectory()
						&& file.getName().toLowerCase().startsWith("taverna-")
						&& !file.getName().toLowerCase().contains("snapshot") // exclude snapshots
						&& !file.getName().toLowerCase().contains("cmd") // exclude command line
																			// tool
		...
		if (previousRegistrationDataFile == null) { // No previous registration file - ask user to
													// register
			UserRegistrationForm form = new UserRegistrationForm(
					applicationConfiguration.getName(), registrationDataFile, doNotRegisterMeFile,
					remindMeLaterFile);
			form.setVisible(true);
			return true;
		} else { // Fill in user's old registration data in the form and ask them to register
			UserRegistrationForm form = new UserRegistrationForm(
					applicationConfiguration.getName(), previousRegistrationDataFile,
					registrationDataFile, doNotRegisterMeFile, remindMeLaterFile);
			form.setVisible(true);
			return true;
		}
	}
