...
public class CopyWorkflowSwingWorker extends SwingWorker<Dataflow, String>{
	...
	public CopyWorkflowSwingWorker(Dataflow dataflowOriginal){
	...
	private Dataflow createDataflowCopy(){
		...
		try {
			dataflowCopy = deserialiser.deserializeDataflow(serialiser
					.serializeDataflow(dataflowOriginal));
		} catch (SerializationException e1) {
			logger .error("Initialising workflow run: Unable to copy workflow", e1);
			dataflowCopy = null;
